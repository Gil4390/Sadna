...
namespace SadnaExpress.DomainLayer.Store
{
    public class Store
    {
        ...
        public List<Item> GetItemsByKeysWord(string keyWords, int minPrice, int maxPrice, int ratingItem,
            string category)
        {
        ...
        public DiscountPolicy.DiscountPolicy AddSimplePolicy<T>(T entity, int percent, DateTime startDate,
            DateTime endDate)
        {
            SimpleDiscount<T> simpleDiscount = new SimpleDiscount<T>(entity, percent, startDate, endDate);
        ...
        public DiscountPolicy.DiscountPolicy AddComplexPolicy(string op, params object[] policys)
        {
            ...
            {
                case "xor":
                    XorDiscount xor = new XorDiscount((DiscountPolicy.DiscountPolicy)policys[0],
                        (DiscountPolicy.DiscountPolicy)policys[1]);
                    return xor;
                case "and":
                    AndDiscount and = new AndDiscount((Condition)policys[0], (Condition)policys[1],
                        (DiscountPolicy.DiscountPolicy)policys[2]);
                    return and;
                case "or":
                    OrDiscount or = new OrDiscount((Condition)policys[0], (Condition)policys[1],
                        (DiscountPolicy.DiscountPolicy)policys[2]);
                    return or;
                case "if":
                    ConditionalDiscount ifCond = new ConditionalDiscount((Condition)policys[0],
                        (DiscountPolicy.DiscountPolicy)policys[1]);
                    return ifCond;
                case "max":
                    MaxDiscount max = new MaxDiscount((DiscountPolicy.DiscountPolicy)policys[0],
                        (DiscountPolicy.DiscountPolicy)policys[1]);
                    return max;
                case "add":
                    AddDiscount add = new AddDiscount((DiscountPolicy.DiscountPolicy)policys[0],
                        (DiscountPolicy.DiscountPolicy)policys[1]);
                    return add;
                default:
